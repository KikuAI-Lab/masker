...
class ChatCompletionsRequest(BaseModel):
    ...
    messages: list[ChatMessage] = Field(
        ..., min_length=1, description="The messages to generate completions for."
    )
    ...
    stream: bool | None = Field(default=False, description="Whether to stream partial responses.")
...
class RedactionMetadata(BaseModel):
    ...
    entities_total: int = Field(
        default=0, description="Total number of PII entities detected and redacted."
    )
    entities_by_type: dict[str, int] = Field(
        default_factory=dict, description="Count of entities by type."
    )
    policy_id: str = Field(default="default", description="Policy ID used for redaction.")
    redaction_ms: float = Field(..., description="Time spent on redaction in milliseconds.")
    total_ms: float = Field(..., description="Total request processing time in milliseconds.")
