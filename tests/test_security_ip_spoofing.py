...
def test_ip_spoofing_rate_limit_bypass_fixed(client):
    ...
    response = client.post("/v1/detect", json={"text": "Hello world"}, headers=headers_1)
    ...
    for _ in range(60):
        client.post("/v1/detect", json={"text": "Hello world"}, headers=headers_1)
    ...
    response = client.post("/v1/detect", json={"text": "Hello world"}, headers=headers_2)
    ...
    assert response.status_code == 429, (
        "Vulnerability persisted: Rate limit bypassed via X-Forwarded-For"
    )
